...
class lifeGame():
	...
	def getGameData(self, max_iter=100, num_games=500, size=20, init_life=100, is_save=True):
	...
	def initGame(self, size=20, init_life=100):
...
class Dataset():
	def __init__(self, path=None, is_shuffle=True):
		if path is None:
			self.X, self.y = lifeGame().getGameData(max_iter=100, num_games=500, size=20, init_life=100, is_save=True)
		else:
